...
void DualArmControl::currentInternalForce(){
       ...
       // Null-Space Projection Matrix
              ...
              Eigen::Matrix<double, 12, 1> internalForce = f_meas;
       ...
       // Null Space projection of squeeze
              ...
              std::cout << "lambda_measured = " << lambdaMeasured_ << std::endl;
...
double DualArmControl::DemandForces(double K,double Fmax) { 
...
const auto & vel_R_sva = robots().robot(rightRobotIndex_).bodyVelW(eeName_);
       ...
       v_obj = 0.5 * (v_left + v_right);

const auto & accB_L = robots().robot(leftRobotIndex_).bodyAccB(eeName_);
const auto & accB_R = robots().robot(rightRobotIndex_).bodyAccB(eeName_);

const auto & pose_L = robots().robot(leftRobotIndex_).bodyPosW(eeName_);
const auto & pose_R = robots().robot(rightRobotIndex_).bodyPosW(eeName_);

Eigen::Vector3d a_left  = pose_L.rotation() * accB_L.linear();
Eigen::Vector3d a_right = pose_R.rotation() * accB_R.linear();
 a_obj = 0.5 * (a_left + a_right);
...
double alpha_v = alpha(v_eff, 0.02, 2);
...
void DualArmControl::optimize(double forceRef) {
    ...
    qpParams.alpha    = 80.0; 
    qpParams.beta     = 1.0;
    qpParams.mu       = 0.2;              // Preso dinamicamente da mc_rtc o dal tuo robot
    double K_demand = 100.0;
    double Fmax = 15.0;

    qpParams.gamma_L = 3.0;              //
    qpParams.gamma_R = 3.0;              // 
   

    // 2. Preparazione Dati di Input per il QP
    DualArmQPOptimizer::InputData qpInput;

    qpInput.xInput = forceRef;
