    ...
    sva::ForceVecd(
        ...
        Eigen::Vector3d(0.0, 0.0, stableContact)));
              ...
              case CollabSubState::TRAJECTORY:{   
                     ...
                     x_0_objectWaypoint1_ = sva::PTransformd(Eigen::Quaterniond(x_0_objectStart_.rotation()),Eigen::Vector3d(0.2, 0.25, 0.));
                     x_0_objectWaypoint2_ = sva::PTransformd(Eigen::Quaterniond(x_0_objectWaypoint1_.rotation()),Eigen::Vector3d(0.6, 0.25, 0.2));
              ...
              case CollabSubState::COOPERATIVE_MOTION:{   
                     ...
                     if(!squeezeForceReached_){
                            ...
                            gains.lambda_desired = lambdaStart + (40.0 - lambdaStart) * s;
                            stateTimer__ += timeStep;
                            
                            leftImpedanceTask_->gains().wrench().vec(Eigen::Vector3d(0,0,0),Eigen::Vector3d(0,0,0.26));
                            rightImpedanceTask_->gains().wrench().vec(Eigen::Vector3d(0,0,0),Eigen::Vector3d(0,0,0.26));
                            if(std::abs(meas - 40.0) < 1){
                                   ++squeezeStableCounter_;
                                   if(squeezeStableCounter_ > 5){
                                          squeezeForceReached_ = true;
                                          motionStarted_ = true;
                                          stateTimer_ = 0.0;
                                          leftImpedanceTask_->gains().wrench().vec(Eigen::Vector3d(0,0,0),Eigen::Vector3d(0,0,0.07));
                                          rightImpedanceTask_->gains().wrench().vec(Eigen::Vector3d(0,0,0),Eigen::Vector3d(0,0,0.07));
                                   }
...
else
{
    ...
    if(!cycleComplete_)
    {
        const sva::PTransformd & targetPose =
            (target_ == 1) ? x_0_objectWaypoint1_
                           : x_0_objectWaypoint2_;

        double posError =
            (x_0_objectCurrent_.translation() - targetPose.translation()).norm();
        ...
        // Reached waypoint
        // ------------------------------------------------------------
        if(!waitingAtWaypoint_ && posError < 0.005)
        {
            x_0_objectStart_ = x_0_objectCurrent_;

            if(target_ == 1)
            {
                // Reached W1 -> start 3 s dwell
                waitingAtWaypoint_ = true;
                waypointDwellTimer_ = 0.0;

                mc_rtc::log::info(
                    "[FSM] Reached W1, waiting {:.1f} s...",
                    waypointDwellDuration_);
            }
            else
            {
                // Reached W2 -> immediately return to W1
                ++w2Visits_;
                ...
                if(w2Visits_ >= maxW2Visits_)
                {
                    ...
                    mc_rtc::log::success(
                        "[FSM] W2 reached: maximum number of visits.");
                }
                else
                {
                    target_ = 1;
        ...
        // Dwell at W1
        // ------------------------------------------------------------
        if(waitingAtWaypoint_)
        {
            waypointDwellTimer_ += timeStep;

            // Keep the object exactly at W1
            x_0_objectCurrent_ = x_0_objectStart_;

            if(waypointDwellTimer_ >= waypointDwellDuration_)
            {
                waitingAtWaypoint_ = false;
                waypointDwellTimer_ = 0.0;
                ...
                mc_rtc::log::info(
                    "[FSM] W1 dwell complete -> moving to W2.");

                w0 = x_0_objectStart_;
                w1 = x_0_objectWaypoint2_;
            }
        }
        else
        {
            ...
            // --------------------------------------------------------
            x_0_objectCurrent_ =
                computeDesiredObjectPose(w1, w0);
        }
    }
}
   ...
   (x_0_objectCurrent_.translation() - x_0_objectWaypoint2_.translation()).norm() < 0.005)
{
    motionFinished_ = true;
                     ...
                     leftImpedanceTask_->gains().spring().vec(Eigen::Vector3d(10,10,10),Eigen::Vector3d(100,100,10));
                     rightImpedanceTask_->gains().spring().vec(Eigen::Vector3d(10,10,10),Eigen::Vector3d(100,100,10));

                     leftImpedanceTask_->gains().damper().vec(Eigen::Vector3d(10,10,10),Eigen::Vector3d(20,20,150));
                     rightImpedanceTask_->gains().damper().vec(Eigen::Vector3d(10,10,10),Eigen::Vector3d(20,20,150));
                     ...
                     x_opt_plot_ = qpOptimizer_.getOptimalForces();
